...
// use App\Http\Controllers\ArticleMailJob;
...
class CommentController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $res = $comment->save();
        if ($res) {
            Mail::to('manskovushka015@gmail.com')->send(new AdminComment($article->name, $comment->text));
            ...
            // $keys = DB::table('cache')->whereRaw('`key` GLOB :key', [':key' => 'comments'.$article->id.'/*[0-9]'])->get();
            // foreach ($keys as $key) {
            //     Cache::forget($key->key);
            // };
            $keys = DB::table('cache')->whereRaw('`key` GLOB :key', [':key' => 'comments*[0-9]'])->get();
